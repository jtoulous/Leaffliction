...
def tab_prediction():
    ...
    with gr.Row():
        ...
        with gr.Column(scale=0, min_width=400):
            ...
            use_range_checkbox = gr.Checkbox(
                label="Use range to limit number of images",
                value=True
            )
            range_test_input = gr.Slider(
                ...
                maximum=500,
                step=1,
                label="Number of images that have been excluded from "
                      "training",
                value=100,
                interactive=True,
                visible=True
            ...
            seed_input = gr.Number(
                label="Seed that was used for splitting data",
                value=42, precision=0, visible=True
            )
            ...
            prediction_button = gr.Button(
                "Run Prediction", variant="primary"
            )

        with gr.Column():
            prediction_accuracy_status = gr.Textbox(
                label="Status",
    ...
    prediction_button.click(
        run_prediction,
        inputs=[
            source_input, agent_input, range_test_input, seed_input,
            use_range_checkbox
        ],
...
def run_prediction(
    source_img, agent_folder, range_test, seed, use_range
):
    ...
    if (model_weights_file is None or agent_file is None or
            model_arch_file is None):
        raise gr.Error(
            "Missing agent files. Please ensure model.weights.h5, "
            "model_architecture.json, and agent.pkl are present."
        )

    # ---------------------------
    #   RUN PRED
    # ---------------------------
    agent = DetectionAgent.load_from_files(
        model_weights_file, agent_file, model_arch_file
    )
    ...
    for cat, cat_imgs in images.items():
        for img_name, img_variants in cat_imgs.items():
            ...
            predictions_list.append(
                f"{icon} {img_name}: {prediction} (True: {cat})"
            )
            ...
            gallery_images.append((
                cv2.cvtColor(img_border, cv2.COLOR_BGR2RGB),
                f"{img_name} | Pred: {prediction} | True: {cat}"
            ))
            ...
            for i, t_img in enumerate(transformed_imgs_bgr):
                ...
                label = (
                    agent.transformations[i]
                    if i < len(agent.transformations)
                    else f"Transfo {i+1}"
                )
                gallery_images.append((
                    cv2.cvtColor(t_img_border, cv2.COLOR_BGR2RGB),
                    f"{label} | Pred: {prediction} | True: {cat}"
                ))
