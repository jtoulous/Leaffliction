...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Dropout, BatchNormalization  # type: ignore # noqa: E402
from tensorflow.keras.utils import Sequence  # type: ignore # noqa: E402
from tensorflow.keras.optimizers import Adam  # type: ignore # noqa: E402

from Transformation import ImgTransformator  # noqa: E402


class DetectionAgent:
    def __init__(self, transfo=['gaussian_blur'], epochs=10, train_size=0.9, img_size=(128, 128), batch_size=32):
        self.transformations = transfo
        self.epochs = epochs
        self.train_size = train_size
        self.img_size = img_size  # Déjà à 128x128 - parfait !
    ...
    def preprocessing(self, X, y=None):
        # Votre code de preprocessing reste inchangé
        if y is not None:
            # Encodage
    ...
    def create_lightweight_model(self):
        """Crée un modèle CNN léger et efficace"""
        model = Sequential([
            ...
            BatchNormalization(),
            MaxPooling2D(2, 2),
            Dropout(0.1),  # Dropout léger pour éviter le surapprentissage
            ...
            # Troisième couche (optionnelle, plus légère)
            ...
            Dropout(0.5),  # Dropout plus fort dans les couches denses
    ...
    def train(self, X, y):
        ...
        # Votre DataManager reste inchangé - c'est parfait !
        ...
        test_data_manager = DataManager(
            ...
            img_size=self.img_size,
            batch_size=self.batch_size,
            transformations=[],  # Pas de transformations pour la validation
            shuffle=False
        )

        # Création du modèle léger
        self.model = self.create_lightweight_model()
        
        # Compilation avec des paramètres optimisés
        optimizer = Adam(learning_rate=0.001)  # Learning rate plus bas pour plus de stabilité
        ...
        # Affichage du résumé du modèle pour vérifier sa légèreté
        print("Architecture du modèle léger :")
        self.model.summary()
    ...
    def predict(self, img):
        # Votre code de prédiction reste inchangé
        X = cv2.resize(img, self.img_size)
        X = X.astype('float32') / 255.0
        X = np.expand_dims(X, axis=0)

        prediction = self.model.predict(X, verbose=0)  # verbose=0 pour moins de logs
    ...
    # Les méthodes save, load, load_from_files restent inchangées
    def save(self, save_folder):
        model_weights_file = os.path.join(save_folder, 'model.weights.h5')
    ...
    def load(load_folder):
        model_weights_file = os.path.join(load_folder, 'model.weights.h5')
...
class DataManager(Sequence):
    # VOTRE DataManager RESTE EXACTEMENT LE MÊME - c'est bon !
