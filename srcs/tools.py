...
def load_original_images(load_folder, progress=None, task=None):
    ...
    Returns:
        dict: A nested dictionary with structure
            {class_name: {image_file: {enhancement_type: image_data}}}
    """
    images_dict = {}
    ...
    if os.path.isfile(load_folder):
        ...
        images_dict[class_name][image_file]['original'] = (
            cv2.imread(load_folder)
        )
    ...
    if os.path.isdir(load_folder):
        ...
        if contents and all(
            os.path.isfile(os.path.join(load_folder, item))
            ...
            if task is not None:
                progress.update(
                    task,
                    total=len([f for f in contents if len(f.split('_')) == 1])
                )
            ...
            total=sum(
                len([
                    f for f in os.listdir(
                        os.path.join(load_folder, leaf_class)
                    )
                    if len(f.split('_')) == 1
                ])
                for leaf_class in os.listdir(load_folder)
                if os.path.isdir(os.path.join(load_folder, leaf_class))
            )
        )
    ...
    for leaf_class in os.listdir(load_folder):
        ...
        for image_file in os.listdir(class_folder):
            if len(image_file.split('_')) == 1:
                ...
                images_dict[leaf_class][image_file]['original'] = (
                    cv2.imread(img_path)
                )
...
def load_images(load_folder, progress=None, task=None):
    ...
    if os.path.isfile(load_folder):
        ...
        if len(image_basename.split('_')) == 1:
            ...
            images_dict[class_name][image_basename]['original'] = (
                cv2.imread(load_folder)
            )
        else:
            ...
            images_dict[class_name][base_name][enhancement] = (
                cv2.imread(load_folder)
            )
    ...
    if task is not None:
        progress.update(
            task,
            total=sum(
                len(os.listdir(os.path.join(load_folder, leaf_class)))
                for leaf_class in os.listdir(load_folder)
                if os.path.isdir(os.path.join(load_folder, leaf_class))
            )
        )
    ...
    for leaf_class in os.listdir(load_folder):
        ...
        for image_file in os.listdir(class_folder):
            ...
            if len(image_basename.split('_')) == 1:
                ...
                images_dict[leaf_class][image_basename]['original'] = (
                    cv2.imread(img_path)
                )
                ...
                images_dict[leaf_class][base_name][enhancement] = (
                    cv2.imread(img_path)
                )
...
def load_images_from_list(load_files_list, original=False,
    ...
    for load_folder in load_files_list:
        if os.path.isfile(load_folder):
            ...
            if class_name.startswith('Unit_test'):
                class_name = (
                    image_file.rstrip('.JPG').rstrip('1').rstrip('2')
                )
                unit_test = True
...
def save_images(images, save_folder, progress=None, task=None):
    ...
    Args:
        images (dict): A nested dictionary with structure
            {class_name: {image_file: {enhancement_type: image_data}}}
    ...
    if task is not None:
        progress.update(
            ...
            total=sum(
                ...
                for variations in class_images.values()
            )
        )
