...
from plantcv.plantcv.homology.x_axis_pseudolandmark import (
...
class ImgTransformator:
    ...
    def transform(self, image=None, progress=None, task=None,
                  display=False, transform=None, transform_list=None):
        """
        Apply transformations to images.

        Args:
            image (np.ndarray, optional): A single image to transform.
                If None, transforms all images in the structure.
            ...
            display (bool, optional): Whether to display images during
                transformation.
            transform (str, optional): Specific transformation to apply.
                If None, applies all transformations.
        ...
        if image is not None:
            ...
            if transform in self.function_map:
                transformed_images[transform] = (
                    self.function_map[transform](image)
                )
            else:
            ...
            total_operations = sum(
                len(imgs) for imgs in self.images_structure.values()
            ) * (num_transformations + 1)
            ...
            for category in self.images_structure:
                ...
                if task is not None:
                    progress.update(
                        task,
                        description=f"↪ Images transformation: {category}"
                    )
                ...
                for img_key in self.images_structure[category]:
                    ...
                    if transform is None and transform_list is None:
                        for trans_name, trans_function in (
                            self.function_map.items()
                        ):
                            self.images_structure[category][img_key][
                                trans_name
                            ] = trans_function(image)
                            if task is not None:
                                progress.update(task, advance=1)
                    ...
                    if display:
                        ...
                        for trans_type, trans_img in (
                            transformed_images.items()
                        ):
                            cv2.imshow(
                                f"{category} - {img_key} - {trans_type}",
                                trans_img
                            )
                        cv2.waitKey(0)
                        cv2.destroyAllWindows()
                ...
                if task is not None:
                    progress.update(
                        task, description="↪ Images transformation"
                    )
    ...
    def gaussian_blur(self, image, k_size=(3, 3)):
        ...
        Args:
            ...
            k_size (tuple, optional): Kernel size for Gaussian blur.
                Default is (3, 3).
        ...
        transformed_image = cv2.cvtColor(
            transformed_image, cv2.COLOR_BGR2GRAY
        )
        transformed_image = cv2.bitwise_and(
            transformed_image, transformed_image, mask=filled_mask
        )
    ...
    def mask(self, image):
        ...
        if contours:
            ...
            contour_mask = np.zeros(
                transformed_image.shape[:2], dtype=np.uint8
            )
            cv2.drawContours(
                contour_mask, [largest_contour], -1, 255,
    ...
    def roi_objects(self, image):
        """
        Draw the region of interest (ROI) around the leaf in the
        image.
        ...
        if contours:
            ...
            cv2.drawContours(
                transformed_image, [largest_contour], -1, (0, 255, 0), 2
            )
            ...
            cv2.rectangle(
                transformed_image, (x, y), (x + w, y + h), (255, 0, 0), 3
            )
    ...
    def pseudolandmarks(self, image):
        ...
        Behavior:
            - Finds the largest contour in the image (assumed to be
              the leaf).
            - Uses PlantCV's x_axis_pseudolandmarks to compute
              pseudolandmarks.
            - Draws the top, bottom, and center vertical pseudolandmarks
              in different colors.
        ...
        if contours:
            ...
            # Use PlantCV's x_axis_pseudolandmarks function
            # (correct signature: img, mask)
            # This function generates pseudolandmarks along horizontal
            # slices
            landmarks_result = x_axis_pseudolandmarks(
                img=transformed_image, mask=leaf_mask
            )

            # Check if the result is valid (not "NA")
            if not (isinstance(landmarks_result, np.ndarray) and
                    landmarks_result.dtype.kind == 'U'):
                ...
                if top is not None and len(top) > 0:
                    for point in top:
                        ...
                        # Red - top boundary
                        cv2.circle(
                            transformed_image, pt, 4, (0, 0, 255), -1
                        )
                ...
                if bottom is not None and len(bottom) > 0:
                    for point in bottom:
                        # Convert to integer tuple (x, y)
                        pt = (int(point[0][0]), int(point[0][1]))
                        # Blue - bottom boundary
                        cv2.circle(
                            transformed_image, pt, 4, (255, 0, 0), -1
                        )
                ...
                if center_v is not None and len(center_v) > 0:
                    for point in center_v:
                        ...
                        # Green - centerline
                        cv2.circle(
                            transformed_image, pt, 4, (255, 255, 0), -1
                        )
    ...
    def spots_isolation(self, image):
        ...
        Behavior:
            - Finds the largest contour (leaf boundary) to remove
              background
            - Converts the image to HSV color space.
            - Dynamically determines brown color thresholds based on
              the image.
            - Creates a mask for brown areas and applies morphological
              operations to enhance it.
            - Returns the image with only the brown areas visible
              within the leaf, non-brown areas set to white
        """
        transformed_image = image.copy()
        contours, _ = self._find_largest_contour(transformed_image)
        ...
        if contours:
            ...
            mask_all_brown = cv2.inRange(
                hsv, np.array([0, 20, 30], dtype=np.uint8),
                np.array([40, 255, 220], dtype=np.uint8)
            )
            ...
            kernel = np.ones((5, 5), np.uint8)
            # Combine d'abord les zones proches
            mask_brown = cv2.morphologyEx(
                mask_brown, cv2.MORPH_CLOSE, kernel
            )
            # Étend les bords
    ...
    def background_removal(self, image):
        ...
        Returns:
            np.ndarray: Image with background removed (white
                background).
        ...
        Behavior:
            ...
            - Applies morphological operations and Gaussian blur to
              smooth edges.
        ...
        if contours:
            ...
            cv2.drawContours(
                precise_mask, [largest_contour], -1, 255,
                thickness=cv2.FILLED
            )
            ...
            mask_3channel = (
                cv2.cvtColor(precise_mask, cv2.COLOR_GRAY2BGR) / 255.0
            )
            transformed_image = (
                image * mask_3channel +
                transformed_image * (1 - mask_3channel)
            ).astype(np.uint8)
    ...
    def _find_largest_contour(self, image):
        """
        Find contours in the image and return them along with the leaf
        mask.
        ...
        Returns:
            tuple: (contours, leaf_mask) where contours is a list of
                detected contours and leaf_mask is the binary mask of
                the leaf area.

        Behavior:
            - Segments the leaf using color thresholds in HSV space.
            - Creates a binary mask of the leaf area, excluding
              shadows.
        ...
        # Green: capture light to dark greens, but exclude very dark
        # (shadows)
        # Increased Value to exclude dark shadows
        ...
        # ADDITIONAL: Exclude very dark regions (shadows) using Value
        # channel
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        # Exclude pixels darker than 50
        _, bright_mask = cv2.threshold(
            gray, 50, 255, cv2.THRESH_BINARY
        )
        # Keep only bright regions
        leaf_mask = cv2.bitwise_and(leaf_mask, bright_mask)
        ...
        # Fill holes
        leaf_mask = cv2.morphologyEx(
            leaf_mask, cv2.MORPH_DILATE, kernel, iterations=1
        )
        # Remove small noise
        leaf_mask = cv2.morphologyEx(
            leaf_mask, cv2.MORPH_ERODE, kernel, iterations=1
        )
...
def ArgumentParsing():
    ...
    parser.add_argument(
        ...
        type=str,
        choices=[
            'gaussian_blur', 'mask', 'roi_objects', 'pseudolandmarks',
            'spots_isolation', 'background_removal'
        ],
        default=None,
...
def range_processing(images, range_nb=None, range_percent=100):
    """
    Limit the number of images to process based on specified number
    and/or percentage.

    Args:
        images (dict): Dictionary of images categorized by class names.
        range_nb (int, optional): Maximum number of images to process.
            If None, no limit is applied.
        range_percent (int, optional): Percentage of images to process
            (0-100). Default is 100.

    Returns:
        dict: Dictionary of images limited to the specified
            number/percentage.

    Behavior:
        - Flattens the images dictionary into a list of (category,
          image_key, image) tuples.
        - Shuffles the list randomly.
        - Selects the first 'range_nb' images if specified.
        - Further limits the selection to 'range_percent' of the total
          images.
    ...
    all_images = (
        all_images[:range_nb] if range_nb is not None else all_images
    )
...
if __name__ == '__main__':
    try:
        ...
        with Progress(
            ...
            images, type_of_load = load_images(
            ...
            # Transform images
            images_transform_task = progress.add_task(
                "↪ Images Transformation", total=0
            )
            transformator = ImgTransformator(images)
            transformed_images = transformator.transform(
                progress=progress, task=images_transform_task,
                display=args.display, transform=args.transform
            )
            progress.update(global_task, advance=1)
            ...
            if args.destination not in [None, '', 'None']:
                images_save_task = progress.add_task(
                    "↪ Save transformed images", total=0
                )
                save_images(
                    transformed_images, args.destination,
